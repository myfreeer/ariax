...
impl<'de> serde::Deserializer<'de> for &mut Deserializer<'de> {
        ...
        V: serde::de::Visitor<'de>,
    {
        ...
        if len > self.input.remaining() {
            return Err(Error::BadMessage("sequence count exceeds frame".to_owned()));
        }
        visitor.visit_seq(SeqDeserializer {
            de: self,
            len: Some(len),
...
impl<'de> SeqAccess<'de> for SeqDeserializer<'_, 'de> {
    ...
    fn size_hint(&self) -> Option<usize> {
        // Never allocate in proportion to an untrusted element count.
        self.len.map(|len| len.min(32))
    }
}
