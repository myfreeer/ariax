...
use crate::{
    client::{run_bounded_tracked, Config, MAX_PACKET_LENGTH, MAX_PENDING_REQUESTS},
    de,
    extensions::{
...
struct RequestState {
    ...
    pending: BTreeMap<Option<u32>, oneshot::Sender<SftpResult<Packet>>>,
}
#[derive(Default)]
struct SharedRequests(Mutex<RequestState>);
impl SharedRequests {
    fn lock(&self) -> std::sync::MutexGuard<'_, RequestState> {
        self.0.lock().unwrap_or_else(|error| error.into_inner())
    }
    fn stop(&self, error: Error) {
        self.lock().closing.get_or_insert(error);
    }
    fn drain(&self, error: Error) {
        let mut state = self.lock();
        let error = state.closing.get_or_insert(error).clone();
...
impl SessionInner {
    pub fn reply(&mut self, id: Option<u32>, packet: Packet) -> SftpResult<()> {
        if let Some(sender) = self.requests.lock().pending.remove(&id) {
...
impl Handler for SessionInner {
    ...
    async fn closed(&mut self, error: Error) {
        self.requests.drain(error);
    }
...
impl RawSftpSession {
    ...
    where
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        let req_map = Arc::new(SharedRequests::default());
        let cancel = CancellationToken::new();
        let packet_cap = Arc::new(AtomicU32::new(cfg.max_packet_len.clamp(1, MAX_PACKET_LENGTH)));
        let inner = SessionInner {
            version: None,
            requests: req_map.clone(),
        };

        let (tx,drained) = run_bounded_tracked(stream,inner,packet_cap.clone(),cancel.clone());
        Self {
            tx,drained,
            cancel,
            packet_cap,
    ...
    pub fn set_limits(&mut self, limits: Limits) {
        if let Some(cap) = limits.packet_len {
            self.packet_cap.fetch_min(cap.clamp(1, MAX_PACKET_LENGTH as u64) as u32, Ordering::AcqRel);
        }
        self.limits = limits;
    }

    pub fn packet_cap(&self) -> u32 {
        self.packet_cap.load(Ordering::Acquire)
    }

    pub fn pending_requests(&self) -> usize {
        self.requests.lock().pending.len()
    }

    /// Closing is sticky; reconnect only after draining this session.
    pub fn is_closed(&self) -> bool {
        self.cancel.is_cancelled() || self.tx.is_closed() || self.requests.lock().closing.is_some()
    }

    /// Close the channel and wait until both protocol tasks released buffers.
    pub async fn drain(&self) {
        self.stop(Error::SessionClosed);
        let mut drained = self.drained.clone();
        while !*drained.borrow_and_update() {
            if drained.changed().await.is_err() { break; }
        }
    }

    fn stop(&self, error: Error) {
        self.requests.stop(error);
        self.cancel.cancel();
    }
    ...
    fn send(
        ...
        if let Some(error) = &state.closing { return Err(error.clone()); }
        if self.tx.is_closed() { return Err(Error::SessionClosed); }
        if state.pending.len() >= MAX_PENDING_REQUESTS { return Err(Error::RequestLimit); }
        if state.pending.contains_key(&id) { return Err(Error::RequestLimit); }
        let bytes = Bytes::try_from(packet)?;
        let max_len = self.limits.packet_len.unwrap_or(MAX_PACKET_LENGTH as u64)
            .min(MAX_PACKET_LENGTH as u64);
        if bytes.len() as u64 > max_len { return Err(Error::PacketTooLarge); }
        let (tx, rx) = oneshot::channel();
        // Serialize admission with close and reply. A sent request always owns a slot.
        self.tx.try_send(bytes).map_err(|error| match error {
            mpsc::error::TrySendError::Full(_) => Error::RequestLimit,
    ...
    async fn request(&self, id: Option<u32>, packet: Packet) -> SftpResult<Packet> {
        let mut rx = self.send(id, packet)?;
        let timeout = self.timeout.load(Ordering::Relaxed);
        match runtime::timeout(Duration::from_secs(timeout), &mut rx).await {
            Ok(Ok(result)) => result,
            Ok(Err(_)) => Err(Error::SessionClosed),
            Err(error) => {
                self.stop(error.clone());
                // Keep the request alive until the decoder has dropped all buffers.
                let _ = rx.await;
                Err(error)
            }
        }
    ...
    pub async fn close<H: Into<Vec<u8>>>(&self, handle: H) -> SftpResult<Status> {
        let id = self.use_next_id();
        let result = self
            .request(
                Some(id),
    ...
    pub async fn read<H: Into<Vec<u8>>>(
        &self,
        handle: H,
        offset: u64,
        len: u32,
    ...
    pub async fn write<H: Into<Vec<u8>>>(
        &self,
        handle: H,
        offset: u64,
        data: Vec<u8>,
    ...
    pub async fn fstat<H: Into<Vec<u8>>>(&self, handle: H) -> SftpResult<Attrs> {
        let id = self.use_next_id();
        let result = self
            .request(
                Some(id),
    ...
    pub async fn fsetstat<H: Into<Vec<u8>>>(
        &self,
        handle: H,
        attrs: FileAttributes,
    ) -> SftpResult<Status> {
    ...
    pub async fn readdir<H: Into<Vec<u8>>>(&self, handle: H) -> SftpResult<Name> {
        let id = self.use_next_id();
        let result = self
            .request(
                Some(id),
    ...
    pub async fn fsync<H: Into<Vec<u8>>>(&self, handle: H) -> SftpResult<Status> {
        let result = self
            .extended(
                extensions::FSYNC,
                FsyncExtension {
