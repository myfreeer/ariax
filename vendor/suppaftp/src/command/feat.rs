...
pub fn parse_features(lines: &[String]) -> Result<Features, FtpError> {
    if lines.len() > crate::MAX_CONTROL_LINES
        || lines.iter().any(|line| line.len() > crate::MAX_CONTROL_LINE)
        || lines.iter().try_fold(0usize, |sum, line| sum.checked_add(line.len())).is_none_or(|sum| sum > crate::MAX_CONTROL_REPLY)
    ...
    if first_line.starts_with("211-") {
        if !lines.last().is_some_and(|line| line.starts_with("211 ")) { return Err(FtpError::BadResponse); }
        // parse list
...
mod test {
    ...
    fn test_should_parse_features_without_end_marker() {
        ...
        assert!(matches!(parse_features(&lines), Err(FtpError::BadResponse)));
    }
