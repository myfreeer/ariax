    ...
    T: TlsStream,
{
    ...
    fn read_response_in(&mut self, expected_code: &[Status]) -> FtpResult<Response> {
        ...
        let result = self.read_bounded_response();
        match result {
    ...
    fn read_bounded_response(&mut self) -> FtpResult<Response> {
    ...
    fn read_line(&mut self, line: &mut Vec<u8>) -> FtpResult<usize> {
        loop {
            let available = self.reader.fill_buf().map_err(FtpError::ConnectionError)?;
