...
contracts and [final-preimplementation-review.md](final-preimplementation-review.md).
...
| R3-20 | [requirements-traceability.md](../project/requirements-traceability.md) stale: no rows for SSRF/redirect/cookie/proxy/rate | Medium | CONFIRMED | G18 |
...
CONFIRMED. [detailed-storage.md](../storage/detailed-storage.md) "Write And Journal Ordering" gives the balanced
...
- State the invariant as a hard rule in [detailed-storage.md](../storage/detailed-storage.md): no record may claim
...
CONFIRMED. [buffer-pool.md](../runtime/buffer-pool.md) allows `HashBorrowed` validation "before or after disk
ack," and [zero-copy.md](../runtime/zero-copy.md) forbids completing a piece before hash validation. But no
...
([security-recovery.md](../architecture/security-recovery.md), [detailed-storage.md](../storage/detailed-storage.md)) has states for in-flight → pending
...
- Document in [detailed-storage.md](../storage/detailed-storage.md) (Recovery + Write ordering) and cross-link from
  [zero-copy.md](../runtime/zero-copy.md).
...
- [split-download.md](../protocols/split-download.md) "Endgame Mode" duplicates a slow range on another mirror and
...
- [redirect-policy.md](../protocols/redirect-policy.md) "Validator And Range Interaction" has a redirect target
...
Both contradict [retry-policy.md](../protocols/retry-policy.md) "Safety Rules," which forbid racing writes of
...
- State the shared rule once (in [retry-policy.md](../protocols/retry-policy.md) Safety Rules) and have
  [split-download.md](../protocols/split-download.md) and [redirect-policy.md](../protocols/redirect-policy.md) reference it, so the three docs stop
  disagreeing.

### R3-4 — SPSC disk-completion lane has multiple producers and is a backpressure inversion

CONFIRMED. [messaging-model.md](../runtime/messaging-model.md) ("SPSC Ring Lanes" / "Default Topology") mandates
`disk completion lane -> SPSC ring -> storage ack`, single-producer/single-consumer.
But [threading-model.md](../runtime/threading-model.md) "Default Sizing" specifies a blocking-disk fallback of
...
- For the blocking-pool fallback, either give each worker its own SPSC ring merged
  ...
  one SPSC ring.
- Update [messaging-model.md](../runtime/messaging-model.md) "Default Topology" so the completion lane's producer
  count matches the disk-worker count in [threading-model.md](../runtime/threading-model.md), and state explicitly
...
- Ownership of the `PieceDurable` append is specified twice: [detailed-storage.md](../storage/detailed-storage.md)
  has the StorageEngine append it inline and emit `WriteAck::PieceDurable`, while
  [detailed-core.md](../architecture/detailed-core.md) "Persistence Hooks" maps a scheduler-level `PieceDurable` event
  to the journal record. Two owners risk double-append or races. [detailed-core.md](../architecture/detailed-core.md)
  ...
  durable.
- Record payloads are unspecified: [detailed-storage.md](../storage/detailed-storage.md) "Control Journal Format"
  ...
  the reset-to-pending step. Segment rotation is named in [session-persistence.md](../storage/session-persistence.md)
  ("append-only or segment-rotated") but [detailed-storage.md](../storage/detailed-storage.md) defines only a single
...
- Name a single serialized journal appender (one task/actor) that assigns `sequence`
  ...
  Remove the inline-append path from [detailed-storage.md](../storage/detailed-storage.md) or the mapped-event path
  from [detailed-core.md](../architecture/detailed-core.md) — keep exactly one.
...
- Define each record's payload layout in [detailed-storage.md](../storage/detailed-storage.md) as the normative
...
type it carries. [buffer-pool.md](../runtime/buffer-pool.md) "Ownership Object" defines the lease as `bytes:
...
[buffer-pool.md](../runtime/buffer-pool.md)'s own "Registered Buffers" requirement that registered memory has
"stable memory addresses until unregistered … cannot be shrunk or released." The
state sets also diverge: [buffer-pool.md](../runtime/buffer-pool.md) lists `DiskDone` and `Free`;
[detailed-runtime.md](../runtime/detailed-runtime.md) drops both and adds `TransformOwned`. [disk-adapter.md](../storage/disk-adapter.md) says
...
- Make [detailed-runtime.md](../runtime/detailed-runtime.md)'s pointer-based form (`ptr`/`capacity`/`owner`) the
  single definition, since it satisfies the stable-address requirement for
  registered buffers; remove the `BytesMut` form from [buffer-pool.md](../runtime/buffer-pool.md) and have it
...
Therefore [configuration.md](../interfaces/configuration.md) is correct to classify the three options as
...
CONFIRMED. [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md) "Resume Offset (REST)" states `REST <offset>` +
...
- Document in [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md) and reconcile with [split-download.md](../protocols/split-download.md)'s
...
[libtorrent-integration.md](../protocols/libtorrent-integration.md) and [configuration.md](../interfaces/configuration.md).

### R3-10 — aria2 `token:<secret>` auth convention is never specified

CONFIRMED. [apis-and-embedding.md](../interfaces/apis-and-embedding.md) and [configuration.md](../interfaces/configuration.md) list `rpc-secret`,
...
Document in [apis-and-embedding.md](../interfaces/apis-and-embedding.md).
...
pump, but [libtorrent-integration.md](../protocols/libtorrent-integration.md) does not account for libtorrent dropping its
...
[libtorrent-integration.md](../protocols/libtorrent-integration.md) Event Channel.
...
`TaskCreated` vs [session-persistence.md](../storage/session-persistence.md)'s SQLite per-task options), and both hold
...
one direction only. Document in [session-persistence.md](../storage/session-persistence.md).

### R3-16 — Runtime-selected async `DiskBackend` needs dyn-compatible or enum dispatch

CORRECTED by R4-4 and the verification pass. [detailed-runtime.md](../runtime/detailed-runtime.md) uses native
...
CONFIRMED. Tokens are acquired before reading ([rate-limiting.md](../runtime/rate-limiting.md)), but short-body,
oversized-body, endgame-loser, and FTP-tail (R3-9) bytes are read-then-discarded,
and [stats-and-stalls.md](../runtime/stats-and-stalls.md) tracks "discarded bytes" separately. Whether discarded
...
the oversized-body and endgame rules). Document in [rate-limiting.md](../runtime/rate-limiting.md).
...
[implementation-readiness.md](../project/implementation-readiness.md) "First Implementation Slice" requires HTTP sequential
download plus live JSON-RPC `addUri`/`tellStatus`/`pause`/`remove`/`getGlobalStat`
"against the real scheduler," but [implementation-plan.md](../project/implementation-plan.md) builds only the
...
meet. Update [implementation-plan.md](../project/implementation-plan.md) and [implementation-readiness.md](../project/implementation-readiness.md).
...
guarantee ([configuration.md](../interfaces/configuration.md), Phase 0 exit, hard invariant #1), but the only stated
mechanism ([detailed-config.md](../interfaces/detailed-config.md): a test-id string per implemented option, plus an
...
Static registry diffing is insufficient; state this in [detailed-config.md](../interfaces/detailed-config.md).

### R3-20 — [requirements-traceability.md](../project/requirements-traceability.md) is stale

CONFIRMED. It does not reference [redirect-policy.md](../protocols/redirect-policy.md), [rate-limiting.md](../runtime/rate-limiting.md),
[detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md), or the five `detailed-*` first-slice docs, and has no rows
...
also leans on [README.md](../architecture/overview.md) as design coverage for Performance/Security/Compactness,
...
CONFIRMED. [configuration.md](../interfaces/configuration.md) replaces `event-poll` with `event-backend`, and
[event-backends.md](../runtime/event-backends.md) defines only `--event-backend`. The value aliases exist
...
[configuration.md](../interfaces/configuration.md).

### R3-21 — Worker sizing oversubscribes small machines

CONFIRMED. [threading-model.md](../runtime/threading-model.md) "Default Sizing": on 2 cores, `network_workers =
...
CONFIRMED. [detailed-storage.md](../storage/detailed-storage.md)/[security-recovery.md](../architecture/security-recovery.md) `SafePathBuilder` reject
...
but did not reconcile the vocabulary. [configuration.md](../interfaces/configuration.md) returns per-option
`OptionRequiresNewGeneration`/`OptionNotRuntimeMutable`; [detailed-config.md](../interfaces/detailed-config.md) returns
...
note, not a defect. [library-choice.md](../architecture/library-choice.md)/[messaging-model.md](../runtime/messaging-model.md) commit to five queue
families (tokio, thingbuf, rtrb, crossbeam, concurrent-queue) and four wrappers,
while [detailed-runtime.md](../runtime/detailed-runtime.md) admits the slice may "start with Tokio bounded channels";
`concurrent-queue` appears in no topology diagram. The HDD/SATA/NVMe latency-
inference adaptivity ([disk-adapter.md](../storage/disk-adapter.md)) is premature before any benchmark baseline
...
CONFIRMED. [detailed-http-first-slice.md](../protocols/detailed-http-first-slice.md) creates the layout and journal before
the request is sent, then permits chunked responses when the total is unknown and
submits each body buffer at an exact offset. [detailed-storage.md](../storage/detailed-storage.md) makes
...
[protocol-modernization.md](../protocols/protocol-modernization.md) permits a decoded body whose length differs from the
...
an explicit commit operation. [split-download.md](../protocols/split-download.md) consequently promises that
...
CONFIRMED. [security-recovery.md](../architecture/security-recovery.md) requires resolving and pinning a target before
connecting and discusses checking the proxy endpoint, while
[protocol-modernization.md](../protocols/protocol-modernization.md) supports HTTP CONNECT and SOCKS5/socks5h remote DNS.
...
CONFIRMED. [configuration.md](../interfaces/configuration.md) lists the user `header` option as implemented, but
...
[redirect-policy.md](../protocols/redirect-policy.md) specifies credential and cookie propagation but not custom
...
CONFIRMED. [detailed-runtime.md](../runtime/detailed-runtime.md) declares `write_at(..., BufferLease) ->
Result<DiskCompletion>` and says the backend must return the same lease on both
completion and error; [disk-adapter.md](../storage/disk-adapter.md) repeats that ownership requirement. The
...
CONFIRMED. [event-backends.md](../runtime/event-backends.md) promises runtime probing among raw epoll, kqueue,
poll, and select backends and exposes an `EventBackend` abstraction.
[library-choice.md](../architecture/library-choice.md) chooses Tokio/Mio for networking and explicitly rejects a
...
CONFIRMED. [detailed-core.md](../architecture/detailed-core.md) defines internal states including `Accepted`,
`Allocating`, `RetryWait`, `PausedSlow`, `Verifying`, `StoppedResult`, and
`Removed`, while [configuration.md](../interfaces/configuration.md) adds `PausedRestarting`. The API document
...
CONFIRMED. [session-persistence.md](../storage/session-persistence.md) stores URI lists, mirror metadata, option
snapshots, validators, and identity state in SQLite/journal artifacts. The
`Secret<T>` rules in [security-recovery.md](../architecture/security-recovery.md) redact logs and may zeroize memory, but
...
CONFIRMED. [metalink-chunking.md](../protocols/metalink-chunking.md) hashes streamed pieces incrementally and avoids
readback, while [split-download.md](../protocols/split-download.md) permits smaller or misaligned dynamic leases
...
CONFIRMED. [detailed-runtime.md](../runtime/detailed-runtime.md) orders global shutdown as network, disk, journal,
session, then BT, which persists the session before the BT session has paused and
saved resume data. [libtorrent-integration.md](../protocols/libtorrent-integration.md) instead requires pause/save-resume,
...
streams notifications, but [messaging-model.md](../runtime/messaging-model.md) only bounds hot internal queues.
...
[configuration.md](../interfaces/configuration.md), referenced by [apis-and-embedding.md](../interfaces/apis-and-embedding.md), with these columns:
...
importantly, round 4 overreads [README.md](../architecture/overview.md): balanced durability batches journal
...
intended to guarantee sparse allocation, [disk-adapter.md](../storage/disk-adapter.md) must define and test the
...
- [event-backends.md](../runtime/event-backends.md) and the repository build remain architectural sources of
