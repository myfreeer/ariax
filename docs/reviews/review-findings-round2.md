...
Historical, non-normative review. Links use the current documentation layout; line numbers and findings refer to the reviewed snapshots. Follow the [current subsystem contracts](../README.md#source-of-truth) when wording differs.
...
[final-preimplementation-review.md](final-preimplementation-review.md). Verdicts and line references below
...
CONFIRMED. [event-backends.md:119-123](../runtime/event-backends.md) defines the trait with `buf: Buffer`,
`read_at -> Buffer`, `fsync(handle, mode: FsyncMode)`, `allocate(...)`, plus
`name()`/`capabilities()`/`open(path, mode)`. [detailed-runtime.md:216-222](../runtime/detailed-runtime.md)
...
- Buffer type is `BufferLease` in both `write_at` and `read_at`. Remove all
  `Buffer` uses from [event-backends.md](../runtime/event-backends.md).
- Keep the union of operations that are actually needed: `open(req: OpenRequest)`,
  ...
  (required by the preallocation modes in [disk-adapter.md](../storage/disk-adapter.md)), `rename(from, to)`
  (required by temp→final finalization in [detailed-storage.md](../storage/detailed-storage.md)), and the backend
...
- Make [detailed-runtime.md](../runtime/detailed-runtime.md) the normative definition (it is the first-slice
  contract). [event-backends.md](../runtime/event-backends.md) states capability/probing semantics and refers to
...
[disk-adapter.md](../storage/disk-adapter.md) `WriteBlock.buffer`, and `Buffer` in the event-backend trait):
`BufferLease` is the single name.

### R2-2 — Control-plane priority is required but no priority primitive is selected

CONFIRMED. [threading-model.md:208](../runtime/threading-model.md) lists "priority for control/journal
messages" as a required channel property and [detailed-runtime.md:51](../runtime/detailed-runtime.md) says
"control/journal priority reserves cannot be consumed by bulk writes."
[backpressure.md:121](../runtime/backpressure.md) says "pause/remove should enqueue immediately." But the
[messaging-model.md:170-178](../runtime/messaging-model.md) selection matrix picks only FIFO/unordered
primitives (`tokio::sync::mpsc`, `thingbuf`, `rtrb`, `crossbeam`,
`concurrent-queue`), and [detailed-runtime.md:89](../runtime/detailed-runtime.md) routes both `addUri` and
...
- `control_bulk`: carries `addUri`/`addTorrent`/status-scan commands, subject to
  admission rejection when budgets are exhausted (this is what [backpressure.md](../runtime/backpressure.md)
...
- State the invariant in [detailed-runtime.md](../runtime/detailed-runtime.md) and [messaging-model.md](../runtime/messaging-model.md): urgent
...
CONFIRMED. [detailed-runtime.md:226-228](../runtime/detailed-runtime.md) quarantines a lease "until completion is
observed" when the OS may still own it after a cancellation; [detailed-runtime.md:82](../runtime/detailed-runtime.md)
sends leaked leases to quarantine on close; the `Quarantine` type
([detailed-runtime.md:110](../runtime/detailed-runtime.md)) has no fields. No document specifies a size cap,
...
metric ([detailed-runtime.md:236](../runtime/detailed-runtime.md)), so the condition is observable, and if
...
- When `quarantine_budget` is exhausted, transition the backend to `Faulted` and
  stop issuing new I/O on it (fall back per [event-backends.md](../runtime/event-backends.md)) rather than
  growing memory. Surface this transition in metrics.

### R2-4 — Cross-mirror split has no entity-identity check without checksums

CONFIRMED. [split-download.md:71](../protocols/split-download.md) runs up to `split=8` non-overlapping leases
across different mirror URIs; [configuration.md:334](../interfaces/configuration.md) only *asserts* that
...
([detailed-http-first-slice.md:156-161](../protocols/detailed-http-first-slice.md)) validate status, `Content-Range`, body
...
- Documented in [split-download.md](../protocols/split-download.md) (Cross-Mirror Entity Identity),
  [detailed-http-first-slice.md](../protocols/detailed-http-first-slice.md), and registered in [configuration.md](../interfaces/configuration.md). Add a
...
R2-5 CONFIRMED. [detailed-storage.md:234-248](../storage/detailed-storage.md) lists `TaskCreated`,
...
`TaskError`, `TaskRemoved`, `CleanShutdown`. [security-recovery.md:139-148](../architecture/security-recovery.md) omits
`OptionsSnapshot`, `GenerationStarted`, `RetryState`, `TaskRemoved`,
`CleanShutdown` and instead adds `PieceStarted`. Worse, [security-recovery.md:153](../architecture/security-recovery.md)
...
R2-6 PARTIAL (real). [session-persistence.md:316-325](../storage/session-persistence.md) describes a third framing
...
([detailed-storage.md:224-225](../storage/detailed-storage.md), [security-recovery.md:134](../architecture/security-recovery.md)). The first-round
"three incompatible formats" was inflated — [detailed-storage.md](../storage/detailed-storage.md) and
[security-recovery.md](../architecture/security-recovery.md) actually agree with each other; the outlier is
[session-persistence.md](../storage/session-persistence.md).

Also in this area, CONFIRMED but low severity: [detailed-storage.md:211-215](../storage/detailed-storage.md)
...
- Make [detailed-storage.md](../storage/detailed-storage.md) the single normative source for the record enum and
  the on-disk layout. [security-recovery.md](../architecture/security-recovery.md) and [session-persistence.md](../storage/session-persistence.md) reference
...
- Adopt the union enum. Keep both `GenerationStarted` (advances the generation)
  ...
  [security-recovery.md:153](../architecture/security-recovery.md) has a concrete referent.
- Remove the `layout hash` field from [session-persistence.md](../storage/session-persistence.md)'s per-record
...
CONFIRMED. `disk-cache` is a user option ([backpressure.md:128](../runtime/backpressure.md)) but appears in
neither the Memory Signals list ([backpressure.md:88-92](../runtime/backpressure.md)) nor any `ResourceManager`
budget ([threading-model.md:55-64](../runtime/threading-model.md), [detailed-runtime.md:35-43](../runtime/detailed-runtime.md)). Total memory can
...
- Add "disk-cache retained bytes" to the Memory Signals list in [backpressure.md](../runtime/backpressure.md).
...
PARTIAL (real). The baseline is correctly conditioned — [protocol-modernization.md:21](../protocols/protocol-modernization.md)
says "gzip/deflate only where range semantics remain correct" — and range workers
require identity ([detailed-http-first-slice.md:160-161](../protocols/detailed-http-first-slice.md)). But no document sends
...
([detailed-http-first-slice.md:108-110](../protocols/detailed-http-first-slice.md)) has no content-encoding rule at all. Byte
...
- Document in the range-worker and resume blocks of [detailed-http-first-slice.md](../protocols/detailed-http-first-slice.md)
  and in the baseline of [protocol-modernization.md](../protocols/protocol-modernization.md).

### R2-9 / R2-15 — Redirect policy is referenced but undefined, including credential handling

CONFIRMED. [detailed-http-first-slice.md:188](../protocols/detailed-http-first-slice.md) and [retry-policy.md:204](../protocols/retry-policy.md) both defer
to a "redirect policy," and [library-choice.md:144](../architecture/library-choice.md) references it, but no
redirect-policy document exists (only [README.md:410](../architecture/overview.md) mentions redirects, in an
...
secret-exfiltration CVE class — and [security-recovery.md](../architecture/security-recovery.md) covers only log
redaction, not header propagation.

Proposed fix F8 — add [redirect-policy.md](../protocols/redirect-policy.md) (and cross-link from
[protocol-modernization.md](../protocols/protocol-modernization.md) and [security-recovery.md](../architecture/security-recovery.md)):
...
PARTIAL (real). [libtorrent-integration.md:87](../protocols/libtorrent-integration.md) does say "downloader validates and
...
internally, versus only the save-root/output path. [security-recovery.md:39](../architecture/security-recovery.md) lists
...
deferred to "a later phase" ([libtorrent-integration.md:97-101](../protocols/libtorrent-integration.md)). So in the
...
- Document this as the interim guarantee in [libtorrent-integration.md](../protocols/libtorrent-integration.md), with the
...
([libtorrent-integration.md:46-48](../protocols/libtorrent-integration.md): backpressure or typed overload error), but the
event channel ([libtorrent-integration.md:50-64](../protocols/libtorrent-integration.md), carrying "piece complete",
...
- Durability- and correctness-critical events (resume-data-ready, terminal
  ...
  than dropping these.
- State the policy in [libtorrent-integration.md](../protocols/libtorrent-integration.md) Event Channel.
...
([retry-policy.md:44-53](../protocols/retry-policy.md)) is HTTP-shaped. FTP and SFTP appear only as one-liners
([README.md:171](../architecture/overview.md), [split-download.md:238](../protocols/split-download.md), [implementation-plan.md:124-129](../project/implementation-plan.md)), yet
...
Proposed fix F11 — add [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md) defining resume offset
...
[README.md:410](../architecture/overview.md) promises "Redirects, proxies, and DNS resolution obey SSRF
guardrails when RPC is remotely exposed," and [requirements-traceability.md](../project/requirements-traceability.md) maps
Security to [security-recovery.md](../architecture/security-recovery.md), but that doc never defines the guardrail.
Remote RPC is a first-class use case ([apis-and-embedding.md](../interfaces/apis-and-embedding.md)), so this is direct
exposure.

Proposed fix F12 — add an SSRF-guardrail section to [security-recovery.md](../architecture/security-recovery.md):
...
`max-upload-limit` are listed "implemented live" ([configuration.md:467](../interfaces/configuration.md)) and a
"token bucket" is mentioned in passing ([stats-and-stalls.md:101](../runtime/stats-and-stalls.md),
[detailed-runtime.md:179](../runtime/detailed-runtime.md)), but no document specifies the limiter: global vs
...
Proposed fix F13 — add [rate-limiting.md](../runtime/rate-limiting.md): token-bucket hierarchy
...
`load-cookies`/`save-cookies` are listed "implemented" ([configuration.md:421-422](../interfaces/configuration.md))
...
Proposed fix F14 — add a cookie-jar section (in [protocol-modernization.md](../protocols/protocol-modernization.md) or a
...
[README.md:417-429](../architecture/overview.md) and [threading-model.md:234-246](../runtime/threading-model.md) require per-host active
connections, per-host recent error rate, and per-task retry causes, plus per-lease
diagnostics ([download-scheduling.md:197-206](../runtime/download-scheduling.md)). At C10k across many hosts/tasks
...
Per-task pause is well specified ([detailed-http-first-slice.md:266-281](../protocols/detailed-http-first-slice.md),
[split-download.md:243-251](../protocols/split-download.md)) and libtorrent has its own shutdown
([libtorrent-integration.md:103-115](../protocols/libtorrent-integration.md)), but there is no global lane-quiescence
...
guarantee during a *normal* shutdown, not just power loss. [detailed-runtime.md](../runtime/detailed-runtime.md)
...
Proposed fix F16 — specify the shutdown sequence in [detailed-runtime.md](../runtime/detailed-runtime.md):
...
[disk-adapter.md:301](../storage/disk-adapter.md) and [retry-policy.md:253](../protocols/retry-policy.md) say ENOSPC is terminal and never
retried, while [backpressure.md:113](../runtime/backpressure.md) lists "pause a task" as a disk-full response,
and the [disk-adapter.md](../storage/disk-adapter.md) `Faulted` state says "fail or pause according to policy"
...
from mid-transfer ENOSPC (pause). Update [disk-adapter.md](../storage/disk-adapter.md), [retry-policy.md](../protocols/retry-policy.md), and
[backpressure.md](../runtime/backpressure.md) to the single policy.

### R2-20 — Proxy CONNECT/SOCKS/auth flows are one-liners (F18)

[detailed-http-first-slice.md:189](../protocols/detailed-http-first-slice.md) handles `407` in one line and
[protocol-modernization.md:238-247](../protocols/protocol-modernization.md) lists proxy interaction abstractly, but the
...
Proposed fix F18 — add a proxy-flow section (in [protocol-modernization.md](../protocols/protocol-modernization.md))
...
- REFUTED — "registered buffers break the memory bound" ([buffer-pool.md](../runtime/buffer-pool.md)). The
  registered arena is a fixed, pre-sized budget bounded like every other class
  ([buffer-pool.md:156-157, 198, 40](../runtime/buffer-pool.md)); the watermark prevents *new* allocation and
...
- REFUTED — "`If-Range` used with weak validators" ([detailed-http-first-slice.md](../protocols/detailed-http-first-slice.md)).
...
- REFUTED — "generation is never defined." [detailed-core.md](../architecture/detailed-core.md) defines a
...
- Downgraded to low — Windows reserved-name list ([security-recovery.md:27](../architecture/security-recovery.md)): the
  list is explicitly illustrative ("such as"), and [detailed-storage.md:53](../storage/detailed-storage.md) refers
...
- Downgraded to low — zero-copy "bypasses `write_block`" ([zero-copy.md](../runtime/zero-copy.md)): the
...
- Downgraded to low — fast-durability "self-contradiction" ([detailed-storage.md](../storage/detailed-storage.md)):
...
- Downgraded to low — option-metadata "violation" ([configuration.md](../interfaces/configuration.md) New Options):
  the typed registry, not the prose, is the source of truth
  ([detailed-config.md:8-10, 52-61](../interfaces/detailed-config.md)) with a CI inventory check
  ([detailed-config.md:104-114](../interfaces/detailed-config.md)). Bare-name listing is the doc's consistent style
...
- Downgraded to low — hook-scope "injection" ([configuration.md](../interfaces/configuration.md)/[security-recovery.md](../architecture/security-recovery.md)):
  `scope` and `runtime_update` are deliberately separate axes
  ([configuration.md:45-50](../interfaces/configuration.md)); hooks are off by default and rejected from URL rules
...
- Downgraded to low — error-taxonomy mismatch (`OptionRequiresNewGeneration` vs
  ...
  but error codes are generated and CI-enforced ([detailed-config.md:96-102](../interfaces/detailed-config.md)), so it
