...
roadmap; gates are tracked in [implementation-readiness.md](../project/implementation-readiness.md).
...
follow [session-persistence.md](../storage/session-persistence.md)'s omission policy.  `--include-secrets` never
changes that policy.
...
afterwards. [detailed-config.md](detailed-config.md) owns the exact application sequence.
...
- FTP/SFTP: `ftp-user`, `ftp-passwd`, `ftp-pasv`, `ftp-reuse-connection`,
  `ftp-type`, `ssh-host-key-md`: implemented per [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md).
  ...
  growing/transformed-output mode in [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md) is implemented.
...
- `piece-length` defaults to 1 MiB for HTTP/FTP, is persisted, and is ignored
  ...
  [detailed-storage.md](../storage/detailed-storage.md) and may return incompatible progress to pending.
- Retry policy: the HTTP worker and option parser implement bounded profiles,
  ...
  `P4-07`. [retry-policy.md#registry-and-persistence-boundary](../protocols/retry-policy.md#registry-and-persistence-boundary) owns the
...
- The checkpoint rejects every non-loopback RPC bind. Any later non-loopback
  support must enforce the startup security policy in [security-recovery.md](../architecture/security-recovery.md).
...
- `event-backend=auto|tokio` selects the single Tokio/Mio network reactor.
  ...
  not accepted by release configuration; see [event-backends.md](../runtime/event-backends.md).
...
- `disk-cache` defaults to `0` and, when nonzero, is the exact verified-span
  readback cache in [buffer-pool.md](../runtime/buffer-pool.md). It is charged inside `buffer_budget`, is
...
- `endgame-max-duplicates` (bounded concurrent duplicate attempts in endgame
  ...
  original lease; see [split-download.md](../protocols/split-download.md))
...
SFTP security (see [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md)):
