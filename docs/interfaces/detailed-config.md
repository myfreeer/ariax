...
[implementation-readiness.md](../project/implementation-readiness.md); additional protocol options remain later work.

This document expands [configuration.md](configuration.md) into concrete artifacts, parsers, and
...
1. read UTF-8 text under the document/line caps in [configuration.md](configuration.md),
2. split lines,
3. trim trailing CR,
...
- document, rule-count, and glob-byte caps are the exact registry/hard values in
  [configuration.md](configuration.md); emitted option maps reserve `task_metadata_budget`,
...
9. for active restart, enter `PausedRestarting`, cancel/drain workers, retain the
   ...
   the sole generation increment point defined by [detailed-core.md](../architecture/detailed-core.md).
...
driver under [session-persistence.md](../storage/session-persistence.md); it must not retry a speculative append.
