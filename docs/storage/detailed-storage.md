...
Implementation and acceptance are tracked in [detailed-protocol-transfers.md](../protocols/detailed-protocol-transfers.md).
...
type and its state machine are defined in [detailed-runtime.md](../runtime/detailed-runtime.md); storage never
...
work, as specified by [detailed-http-first-slice.md](../protocols/detailed-http-first-slice.md).

## Control Journal Format

This is the normative journal spec. [security-recovery.md](../architecture/security-recovery.md) and
[session-persistence.md](session-persistence.md) describe recovery reconciliation and the surrounding
...
stops the driver for recovery under [session-persistence.md](session-persistence.md). Recovery reuses
...
monotonic clock;
[retry-policy.md](../protocols/retry-policy.md) defines how these fields are validated against wall-clock
...
([detailed-runtime.md](../runtime/detailed-runtime.md)). The appender object (sequence counter, tail state,
...
piece-count thresholds in [performance-profiles.md](../runtime/performance-profiles.md), and unconditionally on
...
operation. [session-persistence.md](session-persistence.md) owns its control-plane/API surface; this
