...
- libtorrent can still use its own Asio-based internals behind the BT adapter.
  See [libtorrent-integration.md](../protocols/libtorrent-integration.md) for the isolation model.
...
transfer rather than copied buffers. See [messaging-model.md](../runtime/messaging-model.md).
...
wrappers or OS APIs. See [detailed-runtime.md](../runtime/detailed-runtime.md) and [disk-adapter.md](../storage/disk-adapter.md).
...
in [protocol-modernization.md](../protocols/protocol-modernization.md) are user-visible, included in profile resolution,
...
behavior, and rekey/timeouts are specified in [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md) and exercised
...
depth, attribute, file, and source caps in [configuration.md](../interfaces/configuration.md) and reserve
...
mismatch; [metalink-chunking.md](../protocols/metalink-chunking.md) owns the persistence/verification rule.
...
- Cookie jar: `cookie_store` (0.22.x) with its `public_suffix` feature (backed
  ...
  redirect context in [protocol-modernization.md](../protocols/protocol-modernization.md); `SameSite=None` without
...
- Serialization: `serde`/`serde_json` for RPC, exports, and generated
  ...
  [detailed-storage.md](../storage/detailed-storage.md), never serde.
...
- Secrets: `secrecy` (0.10.x) plus `zeroize` for the `Secret<T>` wrapper
  contract in [security-recovery.md](security-recovery.md).
...
- Rate limiting is project-owned ([rate-limiting.md](../runtime/rate-limiting.md)): `governor` implements
