...
This overview describes the intended architecture and its invariants. The [implementation readiness record](../project/implementation-readiness.md) tracks which contracts have executable evidence and which gates remain open.
...
the earlier `aria2_rust` prototype. [review-findings-response.md](../reviews/review-findings-response.md) is the
...
See [library-choice.md](library-choice.md) for the runtime/library comparison and why this design
chooses Tokio/Mio plus platform disk adapters over libuv, standalone Asio, or
libevent for the Rust core.

See [disk-adapter.md](../storage/disk-adapter.md) for the explicit build-vs-library decision for disk I/O.
...
See [backpressure.md](../runtime/backpressure.md) for how network reads adapt to disk, CPU, and memory
pressure on HDDs, SSDs, NVMe devices, and slower filesystems.

See [threading-model.md](../runtime/threading-model.md) for the split-pool worker model. Event loops, disk I/O,
CPU hashing, and libtorrent are separate lanes by default and coordinated by a
global resource manager.

See [buffer-pool.md](../runtime/buffer-pool.md) for allocation policy, lifecycle, copy targets, and C10k
scaling behavior for transfer buffers.

See [performance-profiles.md](../runtime/performance-profiles.md) for the single user-facing `--profile` setting:
...
See [messaging-model.md](../runtime/messaging-model.md) for the bounded queue choices between worker lanes.
Hot paths use shared pooled buffers and descriptor queues, not copied payloads
or one generic async channel everywhere.

See [split-download.md](../protocols/split-download.md) for dynamic non-overlapping range leases, lease/span
retry, endgame duplication, and sequential fallback behavior.

See [retry-policy.md](../protocols/retry-policy.md) for configurable retry triggers, status-code retry sets,
`Retry-After` handling, bounded waits, and stale connection/validator behavior.

See [redirect-policy.md](../protocols/redirect-policy.md) for redirect depth/loop limits, validator revalidation
across redirects, and cross-origin credential stripping.

See [rate-limiting.md](../runtime/rate-limiting.md) for the token-bucket hierarchy, fairness, precedence over
read-backpressure, and reconciliation with libtorrent's internal limiter.

See [detailed-ftp-sftp.md](../protocols/detailed-ftp-sftp.md) for FTP sequential-resume and SFTP offset rules,
`SIZE`/`MDTM` validators, ASCII-mode prohibition, and FTPS/SFTP transport rules.

See [download-scheduling.md](../runtime/download-scheduling.md) for active/waiting queue policy and optional
slow-slot demotion, where slow tasks can free `max-concurrent-downloads` slots
for queued downloads.

See [metalink-chunking.md](../protocols/metalink-chunking.md) for how Metalink checksum chunks interact with
dynamic range leases and when disk readback is allowed.

See [stats-and-stalls.md](../runtime/stats-and-stalls.md) for monotonic time-based speed sampling and explicit
stalled/backpressured/rate-limited states, so displayed speeds do not freeze
when sockets stop producing packets.

See [apis-and-embedding.md](../interfaces/apis-and-embedding.md) for aria2-compatible RPC, native Rust embedding,
and the optional staged C ABI.

See [configuration.md](../interfaces/configuration.md) for the typed option registry, aria2-style flat config,
optional URL default rules, runtime option mutation, reload, and config dump
policy.

See [protocol-modernization.md](../protocols/protocol-modernization.md) for HTTP/1.1 keep-alive, HTTP/2 multiplexing,
HTTP/3/QUIC, TLS 1.3, ECH, and DoH/DoT policy.

See [session-persistence.md](../storage/session-persistence.md) for the hybrid SQLite plus per-task control
journal decision.

See [implementation-readiness.md](../project/implementation-readiness.md) for the handoff checklist, hard invariants,
generated artifacts, first implementation slice, and prototype-only decisions.
The detailed first-slice module contracts live in [detailed-core.md](detailed-core.md),
[detailed-config.md](../interfaces/detailed-config.md), [detailed-storage.md](../storage/detailed-storage.md), [detailed-runtime.md](../runtime/detailed-runtime.md), and
[detailed-http-first-slice.md](../protocols/detailed-http-first-slice.md).

See [final-preimplementation-review.md](../reviews/final-preimplementation-review.md) for the closed final review record:
...
The [project README](../../README.md#feature-bundles) lists the current feature
bundles. BitTorrent integration in `full` and `compat` follows the
[Phase 6 gates](../protocols/libtorrent-integration.md#phase-6-gates).
...
This is a high-level lifecycle summary. [detailed-core.md](detailed-core.md) is normative for the
...
rules are in [detailed-storage.md](../storage/detailed-storage.md). Session indexing and BitTorrent adapter
resume state are separate persistence domains defined by
[session-persistence.md](../storage/session-persistence.md) and [libtorrent-integration.md](../protocols/libtorrent-integration.md); this overview does not
...
From the `aria2_rust` prototype review ([review-findings-response.md](../reviews/review-findings-response.md)):
