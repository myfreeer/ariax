...
sizing remains roadmap work. [detailed-protocol-transfers.md](detailed-protocol-transfers.md) tracks the current
...
readback. See [metalink-chunking.md](metalink-chunking.md).
...
The full user-configurable retry surface is defined in [retry-policy.md](retry-policy.md).
...
- disk full/quota: set the scheduler `no_space` condition, not a network retry;
  ...
  `BackendUnavailable` fail-closed rule in [disk-adapter.md](../storage/disk-adapter.md).
...
same entity ([configuration.md](../interfaces/configuration.md)); like aria2, this assertion is trusted by
...
- identity content-coding: range/split requests send `Accept-Encoding: identity`,
  ...
  is never a split fallback; see [detailed-http-first-slice.md](detailed-http-first-slice.md).
