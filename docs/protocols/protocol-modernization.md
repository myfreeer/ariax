...
interfaces share that control plane. Phase 5 adds FTP/FTPS and SFTP workers,
Metalink admission and verification with
[passing local evidence](../../performance-evidence/phase5-validation-2026-09-15.md).
HTTP/2, advanced DNS transports, client certificates and BitTorrent remain later
gates. Native Linux acceptance stays deferred until CI is ready and is tracked
in [implementation-readiness.md](../project/implementation-readiness.md).
...
- FTP and SFTP transfer support (correctness contract in [detailed-ftp-sftp.md](detailed-ftp-sftp.md))
...
[detailed-ftp-sftp.md](detailed-ftp-sftp.md).
...
the profile defaults in [performance-profiles.md](../runtime/performance-profiles.md). Pool admission must satisfy
...
crate comparison and condition for reconsideration are in [library-choice.md](../architecture/library-choice.md).
...
- cookies are scoped by host and MUST NOT propagate to unrelated mirror hosts in
  ...
  [split-download.md](split-download.md)),
- cookie propagation across redirects follows [redirect-policy.md](redirect-policy.md): the prior
...
- DNS answer cardinality, positive/negative entry counts, and negative TTL use
  the exact profile/hard caps in [performance-profiles.md](../runtime/performance-profiles.md); after destination
...
- The cookie wrapper enforces the exact total/per-domain entry and byte caps in
  [performance-profiles.md](../runtime/performance-profiles.md) independently of the selected crate. Expired entries
...
[detailed-http-first-slice.md](detailed-http-first-slice.md) and [redirect-policy.md](redirect-policy.md).
