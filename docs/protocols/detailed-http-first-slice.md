...
[apis-and-embedding.md](../interfaces/apis-and-embedding.md) owns its current surface and remaining progress gates.
...
`P4-01` through `P4-10` regressions in [implementation-readiness.md](../project/implementation-readiness.md), including
...
[retry-policy.md#registry-and-persistence-boundary](retry-policy.md#registry-and-persistence-boundary).
...
in [apis-and-embedding.md](../interfaces/apis-and-embedding.md); non-loopback listeners remain unavailable.
...
4. create the output path through the canonical `SafePathInput` /
   `SafePathBuilder` contract in [detailed-storage.md](../storage/detailed-storage.md),
...
followed. Its admission and per-lease exclusivity follow [redirect-policy.md](redirect-policy.md).
...
- `301/302/303/307/308`: redirect policy ([redirect-policy.md](redirect-policy.md)), budgeted
...
[detailed-storage.md](../storage/detailed-storage.md); the HTTP adapter does not create an alternate
...
- normal split under `off` retains the documented aria2-compatible residual
  ...
  require the stronger gates in [split-download.md](split-download.md),
