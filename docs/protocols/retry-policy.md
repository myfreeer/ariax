...
- `ProxyConnect`: a proxy `CONNECT` tunnel or SOCKS handshake failed, distinct
  ...
  [protocol-modernization.md](protocol-modernization.md)).
- `ControlConnection`: an FTP/SFTP control/session connection failed or dropped
  (see [detailed-ftp-sftp.md](detailed-ftp-sftp.md)).
...
Scope rule ([detailed-core.md](../architecture/detailed-core.md) owns the state machine): while any other span of
...
These rules are normative for [split-download.md](split-download.md) and [redirect-policy.md](redirect-policy.md).
