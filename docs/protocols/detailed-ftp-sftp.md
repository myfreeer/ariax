...
- Client download-rate tokens gate the FTP data-channel `read`, not `write_at`.
  ...
  [detailed-storage.md](../storage/detailed-storage.md) without defining another FTP journal format.
...
- Certificate validation follows the same trust configuration as HTTPS
  ([protocol-modernization.md](protocol-modernization.md) TLS).

## SFTP Read Model

- SFTP runs over SSH through russh plus the pinned patched russh-sftp selected
  in [library-choice.md](../architecture/library-choice.md); the adapter must not block network reactor threads.
